...
var spawn2  
var EmptyArray = []
puppet var slave_dedicated_Server = false
...
func setSpawns(player_count):  #this method decides the spawn point of a peer
	if slave_dedicated_Server == false:
...
func _connection_failed():
	...
	get_tree().multiplayer.clear()
	print("Failed to connect to server")

func _connected_to_server():  #Emites on the client when connected to the server
	print("Connected to Server")
	rpc_id(1,'_getPlayer_count',my_id)
